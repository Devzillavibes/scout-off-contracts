...
// #1042: for get_expiring_subscriptions we deliberately place the populated
// expiry buckets ~50,000 days past epoch. A scan that starts at day 0 would
// have to step through ~50,000 empty buckets and blow past any reasonable
// budget; starting at MinExpiryBucketDay touches only the ~20 populated days.
// Budget is estimated pending calibration (see note in PR); it sits far above
// the optimized O(populated-buckets) cost while staying far below what a
// day-0 scan would cost, so a regression is still caught.
const GET_EXPIRING_SUBSCRIPTIONS_CPU_BUDGET: u64 = 1_500_000;
